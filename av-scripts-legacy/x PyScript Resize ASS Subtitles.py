...
def scale_override_tags(line, scale_x, scale_y):
    line = re.sub(
        r'\\fs([\d.]+)', lambda m: f'\\fs{float(m.group(1)) * scale_y:.2f}', line
    )

    line = re.sub(
        r'\\pos\(([\d.]+),([\d.]+)\)',
        lambda m: (
            f'\\pos({float(m.group(1)) * scale_x:.2f},{float(m.group(2)) * scale_y:.2f})'
        ),
        line,
    )
    ...
    def scale_move(m):
        ...
        if m.group(5):
            return (
                f'\\move({x1:.2f},{y1:.2f},{x2:.2f},{y2:.2f},{m.group(5)},{m.group(6)})'
            )
    ...
    line = re.sub(
        r'\\org\(([\d.]+),([\d.]+)\)',
        lambda m: (
            f'\\org({float(m.group(1)) * scale_x:.2f},{float(m.group(2)) * scale_y:.2f})'
        ),
        line,
    )
...
def main():
    folder = questionary.text(
        'Enter path to folder with .ass files:', default='.'
    ).ask()
