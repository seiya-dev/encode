        ...
        if record.nameID == nameID:
            try:
                return (
                    ...
                    else record.string.decode('utf-8')
                )
            except Exception:
                continue
    return None
...
def ask_for_folder():
    return questionary.text(
        '📂 Enter the path to the folder with .ttf/.otf files:'
    ).ask()
...
def rename_fonts_safely(directory):
    ...
    for filename in os.listdir(directory):
        ...
        if file_hash in seen_hashes:
            print(
                f'🔂 Exact duplicate skipped: {filename} (same as {seen_hashes[file_hash]})'
            )
            continue
        ...
        if clean_current.startswith(clean_expected.replace(ext, '')):
            ...
            for existing in seen_names[f'{ps_name}|{version}']:
                ...
                if (
                    ...
                    and calculate_checksum(existing_path) == file_hash
                ):
                    duplicate = True
                    break
            if duplicate:
