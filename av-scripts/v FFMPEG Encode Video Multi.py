...
reName = re.compile(
    r'^(?P<title>.+?( \d|\(\d+\))?)( -)? (?P<episode>(\d+|SP( )?(\d+)|OVA( )?(\d+)?)(v\d+)?)$'
)
reClean = re.compile(
    r'^(\[[^\[\]]*?\]|\([^\(\)]*?\))|(\s?\[[^\[\]]*?\]|\([^\(\)]*?\))+(_rev(\d+)?)?$'
)
    ...
    for x in range(len(outVideoSize)):
        ...
        if subsTrackIndex != '-1' and subsTrackIndex in inSubs.inf:
            ...
            if inSubsInf['ext']:
                ...
                if subsCodec == 'dvd_subtitle' or subsCodec == 'hdmv_pgs_subtitle':
                    ...
                    outsubs = (
                        f"subtitles={outsubs}:stream_index={tid}:fontsdir='{inFonts}'"
                    )
        ...
        if audioTrackIndex != '-1':
            ...
            if len(audioData) > 0 and atrack[0] == '0':
                outAudio += (
                    f'[{audioTrackIndex}] {audioTitle(audioData, int(atrack[1]))}'
                )
                audioCmd.extend(['-map', f'{atrack[0]}:a:{atrack[1]}?', '-c:a'])
                if encodeAudio:
                    outAudio += f' -> aac 2ch {audioBitrate}k'
                    audioCmd.extend(
                        ['aac', '-cutoff', '0', '-b:a', f'{audioBitrate}k', '-ac', '2']
                    )
                else:
                    outAudio += ' -> copy'
...
def configEncode(inPath: Path):
    ...
    setQuality = qtext(
        f'Set Encode {vqType}:', validate=IntValidator, default=vqual
    ).ask()
