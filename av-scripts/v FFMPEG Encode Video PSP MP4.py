...
def doEncode(
    ...
    os.environ['_OUTFILE'] = str(
        os.path.join(pspFolder, f'{PurePath(inFile).stem} [PSP]')
    )
    ...
    if pspEncoderQuality == 'd':
        ...
        x264Params.extend(
            ['-profile:v', 'main', '-level:v', '3.0', '-tune:v', 'animation']
        )
...
def configEncoder(inPath: Path):
    ...
    if pspEncoderMode != 5:
        ...
        if [2, 3, 4].count(pspEncoderMode) > 0:
            anamorphMode = questionary.confirm(
                'Anamorphic Encode? (Default=No) [EXPEREMENTAL]', default=False
            ).ask()
            ...
            if anamorphMode:
                ...
                for sarItem in list(sarModeData.keys()):
                    ...
                    sarOptions.append(
                        Choice(f'{sarIndex}={sarItem}', value=sarOptionValue)
                    )
